...
// ── API secret key — set VITE_API_KEY in your .env file ──────────────────
const API_KEY = import.meta.env.VITE_API_KEY || "";

/** Shared fetch wrapper — attaches the X-API-Key header automatically */
...
/* ════════════════════════════════════════════════════════
   SCORE WHEEL — uses real ratio values from health_data
   ════════════════════════════════════════════════════════ */
function ScoreWheel({ score, health }) {
  ...
  // Use real values from the backend — fall back to 0 if not yet available
  const bars = [
        ...
        {bars.map(b => (
          <div key={b.label} style={{ marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
              ...
              <span style={{ fontSize:11, fontWeight:600, color:col }}>{b.v}%</span>
            </div>
            <div style={{ height:5, background:T.bg4, borderRadius:99, overflow:"hidden" }}>
              <div style={{ height:"100%", width:`${b.v}%`, background:col, borderRadius:99, transition:"width 1.2s ease" }}/>
            </div>
          </div>
        ))}
      </div>
...
/* ════════════════════════════════════════════════════════
   CHAT — DB-backed history, no localStorage, sends user_id
   ════════════════════════════════════════════════════════ */
function ChatSection({ userId }) {
  ...
  // Load history from DB on mount / user change
  useEffect(() => {
    if (!userId) return;
    setFetching(true);
    apiFetch(`/chat/history/${userId}`)
      ...
      .then(d => {
        const h = (d.history || []).map(m => ({ role: m.role, text: m.message }));
        setMsgs(h);
      })
      .catch(() => setMsgs([]))
  ...
  const send = async () => {
    ...
    try {
      ...
      setMsgs(prev => [...prev, { role:"ai", text:"Could not reach the advisor. Check that Flask is running." }]);
    }
    setLoading(false);
  };
  ...
  return (
    <Card>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1rem" }}>
        ...
        {msgs.length > 0 && <Btn variant="ghost" onClick={clearHistory} style={{ fontSize:12, padding:"6px 12px" }}>Clear</Btn>}
      </div>
      ...
      <div style={{ height:340, overflowY:"auto", display:"flex", flexDirection:"column", gap:12, padding:"0.75rem", background:T.bg1, borderRadius:T.r, border:`1px solid ${T.border}`, marginBottom:"0.75rem" }}>
        {fetching && <div style={{ margin:"auto", display:"flex", gap:10, alignItems:"center" }}><Spinner size={16}/><span style={{ fontSize:13, color:T.faint }}>Loading history…</span></div>}
        {!fetching && msgs.length === 0 && <div style={{ margin:"auto", textAlign:"center" }}><div style={{ fontSize:36, opacity:0.14, marginBottom:10 }}>💬</div><div style={{ fontSize:13, color:T.faint }}>Select a suggestion or type your question</div></div>}
        {msgs.map((m, idx) => (
...
/* ════════════════════════════════════════════════════════
   GOAL PLANNER — compound SIP math, 3 CAGR scenarios
   ════════════════════════════════════════════════════════ */
function GoalPlanner({ userId }) {
  ...
  const calc = async () => {
    ...
    try {
      const res = await apiFetch("/goal-plan", {
        method: "POST",
        body: JSON.stringify({ user_id: userId, target_amount: parseFloat(target), time_years: parseFloat(years) }),
      });
      const d = await res.json();
  ...
  return (
    <Card>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:"0.9rem" }}>
        <div><SLabel>Target amount (₹)</SLabel><input type="number" value={target} onChange={e => setTarget(e.target.value)} placeholder="e.g. 500000" min="1"/></div>
        <div><SLabel>Time horizon (years)</SLabel><input type="number" value={years} onChange={e => setYears(e.target.value)} placeholder="e.g. 3" min="0.5" step="0.5"/></div>
      </div>
      ...
      {result && (
        <div className="fu" style={{ marginTop:"1.25rem", display:"flex", flexDirection:"column", gap:"0.9rem" }}>
          ...
          {/* Primary SIP vs current savings */}
          ...
          {/* 3-scenario SIP table */}
          <div style={{ background:T.bg3, border:`1px solid ${T.border}`, borderRadius:T.r, overflow:"hidden" }}>
            <div style={{ padding:"8px 12px", borderBottom:`1px solid ${T.border}` }}><SLabel style={{ marginBottom:0 }}>Required SIP by expected return</SLabel></div>
            {[
              ...
              <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 12px", background:row.active ? T.accentDim : "transparent", borderBottom:i<2?`1px solid ${T.border}`:"none" }}>
                <div>
                  <span style={{ fontSize:12, color:row.active ? T.accent : T.muted, fontWeight:row.active?600:400 }}>{row.label}</span>
                  <span style={{ fontSize:11, color:T.faint, marginLeft:8 }}>{row.cagr} CAGR</span>
                </div>
                <span style={{ fontSize:13, fontWeight:600, color:row.active ? T.accent : T.text }}>₹{fmt(row.sip)}/mo</span>
              </div>
            ))}
          </div>
          ...
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
              <span style={{ fontSize:11, color:T.faint }}>Savings coverage ({result.cagr_moderate} scenario)</span>
              <span style={{ fontSize:11, fontWeight:600, color:result.feasible ? T.green : T.yellow }}>{pct}%</span>
            </div>
            <div style={{ height:5, background:T.bg4, borderRadius:99, overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"flex-start", background:result.feasible ? T.greenDim : "rgba(251,191,36,0.08)", border:`1px solid ${result.feasible ? T.greenBdr : "rgba(251,191,36,0.22)"}`, borderRadius:T.r, padding:"10px 13px" }}>
            ...
            <div>
              <div style={{ fontSize:13, fontWeight:500, color:result.feasible ? T.green : T.yellow, marginBottom:2 }}>{result.feasible ? "Goal is achievable" : "Savings gap detected"}</div>
              <div style={{ fontSize:12, color:T.muted }}>{result.message}</div>
            </div>
          </div>

          {/* Instrument recommendation */}
          <div style={{ background:T.bg3, border:`1px solid ${T.border}`, borderRadius:T.r, padding:"10px 13px" }}>
            <div style={{ fontSize:11, color:T.faint, marginBottom:4 }}>Recommended instruments</div>
            <div style={{ fontSize:13, color:T.accent, fontWeight:500, marginBottom:3 }}>Primary: {result.investment_suggestion}</div>
            <div style={{ fontSize:13, color:T.muted }}>Secondary: {result.secondary_suggestion}</div>
            <div style={{ fontSize:11, color:T.faint, marginTop:6 }}>{result.rationale}</div>
          </div>
        </div>
      )}
    </Card>
        ...
        <div className="fu2">
          ...
          {tab==="overview" && (
            <div className="fu" style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
              {report ? (
                <>
                  <Card><SLabel>Health score</SLabel><div style={{ fontSize:15, fontWeight:500, marginBottom:"1.25rem" }}>Financial wellness overview</div>
                    {/* Pass the full health object so ScoreWheel gets real bar values */}
          ...
          {tab==="ai report" && (
            <div className="fu">
              {report
                ? <Card><SLabel>AI-generated analysis</SLabel><div style={{ fontSize:15, fontWeight:500, marginBottom:"1.25rem" }}>Personalised financial advice</div><MarkdownBlock text={report.ai_report}/></Card>
...
/* ════════════════════════════════════════════════════════
   PROFILE FORM — with min/max validation on inputs
   ════════════════════════════════════════════════════════ */
      ...
      <div style={{ width:"100%", maxWidth:500 }}>
        ...
        <Card cls="fu1" style={{ padding:"2rem" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
            <div><SLabel>Age</SLabel><input type="number" value={form.age} onChange={set("age")} placeholder="e.g. 25" min="1" max="120"/></div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12, marginBottom:12 }}>
            {[["Monthly income (₹)","income"],["Monthly expenses (₹)","expenses"],["Monthly savings (₹)","savings"]].map(([lbl, k]) => (
              <div key={k}><SLabel>{lbl}</SLabel><input type="number" value={form[k]} onChange={set(k)} placeholder="₹ 0" min="0"/></div>
            ))}
          </div>
...
function ProfileCard({ u, active, onSelect, onDelete }) {
  ...
  return (
    <div style={{ marginBottom:3 }}>
      <button onClick={() => { setConfirm(false); onSelect(u); }} style={{ width:"100%", textAlign:"left", padding:"10px 11px", borderRadius:confirm?`${T.r} ${T.r} 0 0`:T.r, background:active?T.bg3:"transparent", border:`1px solid ${active?T.borderMid:"transparent"}`, borderBottom:confirm?`1px solid ${T.border}`:undefined, cursor:"pointer", transition:"all 0.15s", display:"block" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:3 }}>
          <span style={{ fontSize:13, fontWeight:500, color:active?T.text:T.muted }}>Profile #{u.id}</span>
          <div style={{ display:"flex", alignItems:"center", gap:6 }}>
            {active && <span style={{ width:6, height:6, borderRadius:99, background:T.accent }}/>}
            <span onClick={handleDelete} title="Delete profile" style={{ width:20, height:20, borderRadius:6, display:"flex", alignItems:"center", justifyContent:"center", color:confirm?T.red:T.faint, background:confirm?T.redDim:"transparent", border:`1px solid ${confirm?T.redBdr:"transparent"}`, fontSize:11, cursor:"pointer", transition:"all 0.15s", flexShrink:0 }} onMouseEnter={e => { if (!confirm) { e.currentTarget.style.color=T.red; e.currentTarget.style.background=T.redDim; e.currentTarget.style.borderColor=T.redBdr; } }} onMouseLeave={e => { if (!confirm) { e.currentTarget.style.color=T.faint; e.currentTarget.style.background="transparent"; e.currentTarget.style.borderColor="transparent"; } }}>
              {deleting ? "…" : "🗑"}
            </span>
          </div>
        </div>
        <div style={{ fontSize:11, color:T.faint, lineHeight:1.4, marginBottom:6 }}>{(u.financial_goals||"No goal").slice(0,28)}{(u.financial_goals||"").length>28?"…":""}</div>
      ...
      {confirm && (
        <div style={{ background:T.redDim, border:`1px solid ${T.redBdr}`, borderTop:"none", borderRadius:`0 0 ${T.r} ${T.r}`, padding:"8px 11px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:8, animation:"slideDown 0.15s ease both" }}>
          <span style={{ fontSize:11, color:T.red, lineHeight:1.4 }}>Delete Profile #{u.id} and its report?</span>
          <div style={{ display:"flex", gap:6, flexShrink:0 }}>
            <button onClick={handleDelete} disabled={deleting} style={{ background:T.red, color:"#0c0e12", border:"none", borderRadius:6, padding:"4px 10px", fontSize:11, fontWeight:600, cursor:deleting?"not-allowed":"pointer", opacity:deleting?0.6:1 }}>{deleting?"Deleting…":"Yes, delete"}</button>
            <button onClick={e => { e.stopPropagation(); setConfirm(false); }} style={{ background:"transparent", color:T.muted, border:`1px solid ${T.border}`, borderRadius:6, padding:"4px 10px", fontSize:11, cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:T.bg0, fontFamily:T.font }}>
      <Sidebar users={users} activeUser={activeUser} onSelect={u => { setActive(u); setPage("dashboard"); }} onNewProfile={() => setPage("profile")} onDelete={handleDelete} page={page}/>
